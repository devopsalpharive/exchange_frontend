...
/**ACTIONS */
import { getOrderBook } from "../../actions/DerivativeAction";
...
const OrderBook = () => {
  ...
  useEffect(() => {
    if (!isEmpty(futuresPair) && SpotId.current != futuresPair._id) {
      SpotId.current = futuresPair._id
    }
  }, [futuresPair])
  ...
  useEffect(() => {
    socketContext.socket.on("DerivativeorderBook", (result) => {
      let tikerRoot = localStorage.getItem('futurespair')
        ...
        <div className="ob_table_head">
          <div className="row mx-auto align-items-center">
            <div className="col-4 px-0">
              <p className="text-start">Price ({futuresPair.secondCurrency})</p>
            </div>
            <div className="col-4 px-2">
              <p className="text-center">Amount ({futuresPair.firstCurrency})</p>
            </div>
            <div className="col-4 px-0">
              <p className="text-end">Total ({futuresPair.secondCurrency})</p>
            </div>
          </div>
        </div>
        <div className="ob_table_body">
          {(layout == "all" || layout == "loss") &&
            <div className="ob_table_loss d-flex flex-column-reverse  justify-content-start"
              ref={listRef}
            ...
            >
              {orderBook?.sellOrder?.length > 0
                ? orderBook?.sellOrder?.map((val, i) => {
                  ...
                  return (
                    <div id="sell" className={`ob_table_loss_cont position-relative orderbookprices ${sellhover >= i && "orderbookbg sellhover"}`} onMouseEnter={() => handleMouseEnter(val, i, "sell")} onMouseLeave={() => setSellhover()}>
                      ...
                      <div className="row mx-auto">
                        ...
                        <div className="col-4 px-2">
                          <p className="text-center">
                            {isNaN(val?.quantity)
                              ? 0
                              : val?.quantity?.toFixed(futuresPair?.secondDecimal)}
                          </p>
                        </div>{" "}
                        <div className="col-4 px-0">
                          <p className="text-end">
                            {isNaN(val?.total)
                              ? 0
                              : val?.total?.toFixed(futuresPair?.firstDecimal)}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="ob_table_profit_heads" >
              <div className="row mx-auto">
                <div className="col-4 px-0 d-flex flex-column align-items-start">
                  <p>Last Price</p>
                  <p className="ob_table_prft_value mt-1">
                    {isNaN(futuresTicker.last)
                      ? 0
                      : futuresTicker?.last?.toFixed(futuresPair?.secondDecimal)}
                  </p>
                </div>
                ...
                <div className="col-4 px-0 d-flex flex-column align-items-end">
                  ...
                  <p
                    className={
                      futuresTicker.change > 0
                        ? "ob_change_val val_green_txt"
                        : "ob_change_val val_red_txt"
                    }
                  >
                    {isNaN(futuresTicker?.change)
                      ? 0
                      : futuresTicker?.change?.toFixed(2)}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="ob_table_profit " >
              {orderBook?.buyOrder?.length > 0
                ? orderBook?.buyOrder?.map((val, i) => {
                  ...
                  var precentage = (val.totalAmount / toam) * 100;
                  return (
                    <div id="buy" className={`ob_table_profit_cont position-relative orderbookprices ${i < buyhover && "orderbookbg buyhover"}`} onMouseEnter={() => handleMouseEnter(val, i, "buy")} onMouseLeave={() => setBuyhover()}>
                      <div
                      ...
                      <div className="row mx-auto">
                        ...
                        <div className="col-4 px-2">
                          <p className="text-center">
                            {isNaN(val?.quantity)
                              ? 0
                              : val?.quantity?.toFixed(futuresPair?.firstDecimal)}
                          </p>
                        </div>{" "}
                        <div className="col-4 px-0">
                          <p className="text-end">
