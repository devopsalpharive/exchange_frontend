...
import { getCloseOrders } from "../../actions/FuturesAction";
...
const CloseOrders = () => {
  ...
  useEffect(() => {
    socketContext.socket.on("FuturesClosedPosition", (result) => {
    ...
    <div className="">
      ...
      <div className="custom_tableWrap table_body_white">
        <table className="table">
          <thead>
            <tr>
              ...
              <th className="text-center">Balance ({futuresPair.profitCurrency})</th>
            </tr>
          </thead>
          <tbody>
            {CloseOrder.length > 0 ? (
                    ...
                    <td className="text-center">
                      {isNaN(record?.entryPrice)
                        ...
                        : parseFloat(record?.entryPrice)?.toFixed(
                          futuresPair?.secondDecimal
                        )}{" "}
                      {futuresPair?.secondCurrency}
                    </td>
                    <td className="text-center">
        ...
        <div className="pt-4 pb-3 d-flex align-items-center justify-content-center">
          {count != CloseOrder?.length && CloseOrder?.length > 0 && nextPage ? (
            <button
