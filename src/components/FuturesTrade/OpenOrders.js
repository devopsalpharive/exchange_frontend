...
import { CancelTrade, getOpenOrders } from "../../actions/FuturesAction";
...
const OpenOrders = () => {
  ...
  useEffect(() => {
    socketContext.socket.on("FuturesopenOrder", (result) => {
                  ...
                  <tr>
                    ...
                    <td className="text-center">
                      {isNaN(val?.orderValue)
                        ? 0
                        : val?.orderValue?.toFixed(futuresPair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
                    <td className="text-center">
                      {isNaN(val?.takeProfitPrice) || val?.takeProfitPrice == 0
                        ? "-"
                        : `${val?.takeProfitPrice?.toFixed(futuresPair.secondDecimal)} ${val?.secondCurrency}`}{" "}

                    </td>
                    <td className="text-center">
                      {isNaN(val?.stopLossPrice) || val?.stopLossPrice == 0
                        ? "-"
                        : `${val?.stopLossPrice?.toFixed(futuresPair.secondDecimal)} ${val?.secondCurrency}`}{" "}
