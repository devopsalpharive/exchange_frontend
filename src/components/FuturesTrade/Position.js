...
import { getuserPositionOrders, ClosepositionOrder } from "../../actions/FuturesAction";
...
const PositionOrders = () => {
  ...
  const SpotId = useRef(futuresPair?._id);
  const [show, setShow] = useState(false)
  const [closeData, setCloseData] = useState({})
  const [errors, setErrors] = useState({})
  // console.log(futuresPair, "futuresPair");
  ...
  useEffect(() => {
    if (!isEmpty(futuresTicker)) {
      ArrangePositionOrder(futuresTicker)
    }
  }, [futuresTicker])
  ...
  useEffect(() => {
    socketContext.socket.on("FuturesPositionOrder", (result) => {
    ...
    try {
      if (positionOrder.length > 0) {
        ...
        PositionData.map((val, i) => {
          if (val.pairName == futuresPair.symbol) {
            ...
            if (futuresPair.secondCurrency != futuresPair.profitCurrency) {
              val['grossPandL'] = ConvertingAmount(futuresPair.secondCurrency, futuresPair.profitCurrency, val.grossPandL, priceConversionList)
            }
            // val['closeFee'] = (Math.abs(parseFloat(val.grossPandL)) * parseFloat(futuresPair.close_fee) / 100)
                ...
                return (
                  <tr>
                    ...
                    <td className="text-center">
                      {isNaN(val?.price)
                        ? 0
                        : val?.price?.toFixed(futuresPair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
                    <td className="text-center">
                      {isNaN(val.close_price)
                        ? 0
                        : val.close_price?.toFixed(futuresPair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
                    <td className="text-center">
                      {isNaN(val.takeProfitPrice)
                        ? 0
                        : val.takeProfitPrice?.toFixed(futuresPair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
                    <td className="text-center">
                      {isNaN(val.stopLossPrice)
                        ? 0
                        : val.stopLossPrice?.toFixed(futuresPair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
                    <td className={
                      val.grossPandL > 0
                      ...
                      {isNaN(val.grossPandL)
                        ? 0
                        : val.grossPandL?.toFixed(futuresPair.secondDecimal)}{" "}
                      {futuresPair?.profitCurrency}
                    </td>
