...
import { SPOT_ORDERBOOK_PRICE } from "../../constant/Index";
...
const OrderBook = () => {
  ...
  const formatDynamicNumber = (input) => {
    try {
      let strCheck = input.toString();
      if (strCheck.startsWith("0.00000") || strCheck.startsWith("0.0000")) {
        let str = parseFloat(input).toFixed(20).replace(/0+$/, '');
        ...
        let significant = str.replace(/^0\.(0+)/, '');
      ...
      console.log("formatDynamicNumber_err", e);
    }
  }
  const priceRef = useRef(null);
  // const handlePriceRefClick = () => {
  //   priceRef.current
  // }
  ...
  const fetchOrderBook = async () => {
    try {
      if (!isEmpty(SpotId.current)) {
        ...
        if (status) {
          console.log(result, 'fetchOrderBook')
          let buyTotal = 0
          let sellTotal = 0
  ...
  console.log('orderbooksell', orderBook?.sellOrder)
  ...
  return (
    <div className="trade_card orderbooktable">
      ...
      <div className="orderlayoutsec">
        {/* <img src={Images.layouticon1} className={layout == "all" && "active"} onClick={() => layoutfn("all")} />
        <img src={Images.layouticon2} className={layout == "loss" && "active"} onClick={() => layoutfn("loss")} />
        <img src={Images.layouticon3} className={layout == "profit" && "active"} onClick={() => layoutfn("profit")} /> */}
      ...
      <div className={`${(layout == "loss" || layout == "profit") && "ob_table_ht"} ob_table mt-0 position-relative`}>
        <div className="ob_table_head">
          <div className="row mx-auto align-items-center">
            <div className="col-3 px-0">
              <p className="text-start">Price ({spotPair?.secondCurrency})</p>
            </div>
            <div className="col-5 px-2">
              <p className="text-end">Amount ({spotPair?.firstCurrency})</p>
            </div>
            <div className="col-4 px-0">
              <p className="text-end">Total ({spotPair?.secondCurrency})</p>
            </div>
          </div>
        </div>
        <div className="ob_table_body position-relative">
          ...
          {(layout == "all" || layout == "loss") &&
            <div
              className="ob_table_loss d-flex flex-column-reverse justify-content-start px-0"
              ref={listRef}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="row mx-auto">
                        <div className="col-3 px-0">
                          <p className="text-start loss_value">{formatDynamicNumber(val?._id)}</p>
                        </div>
                        <div className="col-5 px-2">
                        ...
                        <div className="col-4 px-0">
                          <p className="text-end">
                            {isNaN(val?.total)
                              ? 0
                              : val?.total?.toFixed(spotPair?.firstDecimal)}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <div className="row mx-auto">
                <div className="col-4 px-0 d-flex flex-column align-items-start">
                  ...
                  <p className="ob_table_prft_value ">
                    {isNaN(spotTicker.last)
                      ? 0
                      : spotTicker?.last?.toFixed(spotPair?.secondDecimal)}
                  </p>
                </div>
                ...
                <div className="col-4 px-0 d-flex flex-column align-items-end">
                  ...
                  >
                    {isNaN(spotTicker?.change)
                      ? 0
                      : spotTicker?.change?.toFixed(2)}
                  </p>
                </div>
              </div>
            </div>
                ...
                ? orderBook?.buyOrder?.map((val, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="row mx-auto" >
                        <div className="col-3 px-0">
                          <div className="text-start profit_value" name="sdfsd" >{formatDynamicNumber(val?._id)}</div>
                        </div>
                        <div className="col-5 px-2">
                          <p className="text-end">
                            {isNaN(val?.quantity)
        ...
        {/* <div className="hide_ob_overflow_bottom"></div> */}
