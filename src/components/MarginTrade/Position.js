...
import { getuserPositionOrders, ClosepositionOrder } from "../../actions/MarginAction";
...
import { getPorfitCalculation } from "../../lib/Calculationlib";
...
const PositionOrders = () => {
  ...
  // console.log(marginPair, "marginPair");
  ...
  useEffect(() => {
    // console.log(marginTicker, 'marginTicker')
    if (!isEmpty(marginTicker)) {
      ArrangePositionOrder(marginTicker)
    }
  }, [marginTicker])
  ...
  useEffect(() => {
    socketContext.socket.on("MarginPositionOrder", (result) => {
      // console.log("MarginPositionOrder", result.data)
      setBPositionOrder(result.data)
    ...
    try {
      if (positionOrder.length > 0) {
        ...
        PositionData.map((val, i) => {
          if (val.pairName == marginPair.symbol) {
            ...
            // val['closeFee'] = (Math.abs(parseFloat(val.grossPandL)) * parseFloat(marginPair.close_fee) / 100)
            // val['profirAndLoss'] = parseFloat(val.grossPandL) - parseFloat(val.closeFee)
            return val
          }
        })
        console.log(positionOrder, 'ArrangePositionOrder')
  ...
  return (
    <div className="table_div">
      ...
      {console.log(positionOrder, 'positionOrder1', BpositionOrder)}
      <div className="custom_tableWrap table_body_white">
        <table className="table">
          ...
          <tbody>
            {positionOrder && positionOrder.length > 0 ? (
                ...
                return (
                  <tr>
                    ...
                    <td className="text-center">
                      {parseFloat(val.positionFilled, marginPair.firstDecimal)} / {parseFloat(val.balanceFilled, marginPair.firstDecimal)}
                    </td>
                    <td
                      className={
                        val.buyorsell == "buy"
                    ...
                    <td className="text-center">
                      {isNaN(val.close_price)
                        ? 0
                        : val.close_price?.toFixed(marginPair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
                    <td className="text-center">
                      {isNaN(val.takeProfitPrice)
                        ? 0
                        : val.takeProfitPrice?.toFixed(marginPair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
                    <td className="text-center">
                      {isNaN(val.stopLossPrice)
                        ? 0
                        : val.stopLossPrice?.toFixed(marginPair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
                    <td className={
                      val.grossPandL > 0
                      ...
                      {isNaN(val.grossPandL)
                        ? 0
                        : val.grossPandL?.toFixed(marginPair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
      ...
      {positionOrder.length <= 0 && <div className="table_nodata">
