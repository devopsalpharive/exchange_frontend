...
/**ACTION */
import { OrderPlacing } from "../../../actions/DerivativeAction";
...
const LimitCards = ({ buyorsell, orderType }) => {
  ...
  const handleChange = (e) => {
    try {
      ...
      let orderValue
      let price = orderType == 'market' || orderType == 'stop' ? derivativePair.marketPrice : FormValue.price
      ...
      if (name == 'price') {
        if (balPrecen > 0) {
          ...
          orderValue = isNaN(orderValue) ? 0 : parseFloat(orderValue).toFixed(derivativePair.secondDecimal)
  ...
  const percentageValue = (value, FormValue) => {
    try {
      if (buyorsell == 'buy') {
        let buyquantity = 0
        let investBalance = (secondCurrency.derivativeBalance * (balPrecen / 100))
        buyquantity = investBalance / parseFloat(value)
        buyquantity = isNaN(buyquantity) ? 0 : parseFloat(buyquantity).toFixed(derivativePair.firstDecimal)
  ...
  const handleBalanceprecn = (percentage) => {
    try {
      ...
      let FormValue = { ...formValue }
      let price = orderType == 'market' || orderType == 'stop' ? derivativeTicker.marketPrice : formValue.price
      if (buyorsell == 'buy') {
        let buyquantity = 0
        let investBalance = (secondCurrency.derivativeBalance * (parseFloat(percentage) / 100))
        ...
        let sellquantity = 0
        sellquantity = (firstCurrency.derivativeBalance * (parseFloat(percentage) / 100))
        sellquantity = isNaN(sellquantity) ? 0 : parseFloat(sellquantity).toFixed(derivativePair.firstDecimal)
  ...
  const InitalPriceSet = () => {
    let FormValue = { ...formValue }
    FormValue = { ...FormValue, ['price']: derivativeTicker.marketPrice }
    setFormValue(FormValue)
    LiqPriceSet(FormValue)
  }
  ...
  const SetInitalData = () => {
    try {
      ...
      FormValue = { ...FormValue, ['price']: derivativeTicker.marketPrice, ['leverage']: 1 }
      setFormValue(FormValue)
      setFee('0')
      setisEdit(false)
      setOrderCost('0')
  ...
  const feeCalculation = (order_cost) => {
    try {
      let fee = order_cost * (derivativePair.open_fee / 100)
      fee = isNaN(fee) ? 0 : fee
      fee = parseFloat(fee).toFixed(derivativePair.feePrec)
      setFee(fee)
    } catch (err) {
  ...
  const OrderCostSet = (formvalue) => {
    try {
      ...
      order_cost = parseFloat(order_cost).toFixed(derivativePair.secondDecimal)
      console.log(order_cost, 'OrderCostSet')
      if (derivativePair.secondCurrency != derivativePair.profitCurrency) {
        order_cost = ConvertingAmount(derivativePair.secondCurrency, derivativePair.profitCurrency, order_cost, priceConversionList)
        order_cost = isNaN(order_cost) ? 0 : order_cost
        order_cost = parseFloat(order_cost).toFixed(derivativePair.secondDecimal)
      }
  ...
  const LiqPriceSet = (formvalue) => {
    try {
      ...
      let liqPrice = calculateLiquidityPrice({
        price: formvalue.price,
        leverage: formvalue.leverage,
        maintMargin: derivativePair.maintenanceMargin,
        type: buyorsell == 'buy' ? 'long' : 'short'
      })
      liqPrice = isNaN(liqPrice) ? 0 : liqPrice
      liqPrice = parseFloat(liqPrice).toFixed(derivativePair.secondDecimal)
  ...
  const SetTakeProfit = (takeProfitPrice) => {
    try {
      ...
      let percentage = 0
      if (buyorsell == 'buy') {
        percentage = ((parseFloat(takeProfitPrice) - parseFloat(derivativePair.marketPrice)) / parseFloat(derivativePair.marketPrice)) * 100
      } else if (buyorsell == 'sell') {
        percentage = ((parseFloat(derivativePair.marketPrice) - parseFloat(takeProfitPrice)) / parseFloat(derivativePair.marketPrice)) * 100
      }
  ...
  const SetStopLoss = (stopLossPrice) => {
    try {
      ...
      if (buyorsell == 'buy' && stopLossPrice != 0) {
        percentage = ((parseFloat(derivativePair.marketPrice) - parseFloat(stopLossPrice)) / parseFloat(derivativePair.marketPrice)) * 100
      } else if (buyorsell == 'sell' && stopLossPrice != 0) {
        percentage = ((parseFloat(stopLossPrice) - parseFloat(derivativePair.marketPrice)) / parseFloat(derivativePair.marketPrice)) * 100
      }
      console.log(percentage, 'SetTakeProfit')
      percentage = isNaN(percentage) ? 0 : percentage
      percentage = isNaN(percentage) ? 0 : percentage
  ...
  const SetGainAndLoss = (type, value) => {
    try {
      ...
      if (buyorsell == 'buy' && type == 'isTake') {
        tpandSp = parseFloat(derivativePair.marketPrice) + ((parseFloat(derivativePair.marketPrice) * parseFloat(value)) / 100)
        setGainPercen(value)
  ...
  const AssetSet = () => {
    try {
      if (!isEmpty(derivativePair)) {
        // let firstCurrencyData = userAsset?.assets?.find(
        //   (val) => val.currencyId.toString() == derivativePair?.firstCurrencyId
        // );
        // setfirstCurrency(firstCurrencyData)
        // let secondCurrencyData = userAsset?.assets?.find(
        //   (val) => val.currencyId.toString() == derivativePair?.secondCurrencyId
        // );
        // setsecondCurrency(secondCurrencyData)

        let firstCurrencyData = userAsset?.assets?.find(
          (val) => val._id.toString() == derivativePair?.firstCurrency
        );
        setfirstCurrency(firstCurrencyData)
        let secondCurrencyData = userAsset?.assets?.find(
          (val) => val._id.toString() == derivativePair?.secondCurrency
        );
        setsecondCurrency(secondCurrencyData)
      }
  ...
  // useEffect(() => {
  //   if (!isEmpty(derivativeTicker) && !isEdit) {
  //     InitalPriceSet()
  //   }
  // }, [derivativeTicker])
  ...
  useEffect(() => {
    console.log('AssetSet', userAsset, tikerRoot)
      ...
      <div className="">
                ...
                <div className="col-4 col-sm-2 col-md-3 col-lg-2 col-xl-4 col-xxl-3 col-xxxl-2 px-0">
                  <div className="coin_select">
                    <input
                      type="text"
                      className="trade_value_input w-100"
                      placeholder={derivativePair.secondCurrency}
                      disabled={true}
                    />
                  </div>
                </div>
              ...
              <div className="col-4 col-sm-2 col-md-3 col-lg-2 col-xl-4 col-xxl-3 col-xxxl-2 px-0">
                <div className="coin_select">
                  <input
                    type="text"
                    className="trade_value_input w-100"
                    placeholder={derivativePair.firstCurrency}
                    disabled={true}
                  />
                </div>
              </div>
              ...
              {formValue?.isTake &&
                <>
                  <div className="input-group mb-3">
                    ...
                    />
                    <span className="input-group-text" id="basic-addon2">{derivativePair.secondCurrency}</span>
                  </div>
        ...
        <div className="mt-4 trade_values_p">
          <div className="row mb-2">
            ...
            <div className="col-7 col-sm-8 col-xl-6 col-xxl-7 col-xxxl-8 d-flex justify-content-end">
              <p style={{ wordBreak: "break-all" }} className="text-end">{buyorsell != 'buy' ?
                `${isEmpty(firstCurrency?.derivativeBalance) ? 0 : parseFloat(firstCurrency?.derivativeBalance)?.toFixed(firstCurrency?.pip)} ${derivativePair?.firstCurrency}` :
                `${isEmpty(secondCurrency?.derivativeBalance) ? 0 : parseFloat(secondCurrency?.derivativeBalance)?.toFixed(secondCurrency?.pip)} ${derivativePair?.secondCurrency}`}</p>
            </div>
          </div>
          <div className="row mb-2">
            <div className="col-5 col-sm-4 col-xl-6 col-xxl-5 col-xxxl-4">
            ...
            <div className="col-7 col-sm-8 col-xl-6 col-xxl-7 col-xxxl-8 d-flex justify-content-end">
              <p style={{ wordBreak: "break-all" }} className="text-end">{formValue.orderValue} {derivativePair.secondCurrency}</p>
            </div>
          </div>
          <div className="row mb-2">
            <div className="col-5 col-sm-4 col-xl-6 col-xxl-5 col-xxxl-4">
            ...
            <div className="col-7 col-sm-8 col-xl-6 col-xxl-7 col-xxxl-8 d-flex justify-content-end">
              <p style={{ wordBreak: "break-all" }} className="text-end">{liqPrice} {derivativePair.secondCurrency}</p>
            </div>
          </div>
          <div className="row">
            <div className="col-5 col-sm-4 col-xl-6 col-xxl-5 col-xxxl-4">
            ...
            <div className="col-7 col-sm-8 col-xl-6 col-xxl-7 col-xxxl-8 d-flex justify-content-end">
              <p style={{ wordBreak: "break-all" }} className="text-end">{orderCost} {derivativePair.profitCurrency}</p>
            </div>
          </div>
          <div className="row mt-2">
            ...
            <div className="col-7 col-sm-8 col-xl-6 col-xxl-7 col-xxxl-8 d-flex justify-content-end">
              <p style={{ wordBreak: "break-all" }} className="text-end">{`${Fee} ${derivativePair.profitCurrency}`}</p>
            </div>
          </div>
        </div>
        ...
        <button
          ...
          onClick={() => {
            if (!isEmpty(getUser)) {
              ...
              sessionStorage.setItem('loginFrom', '/derivative-trading')
              navigate('/login')
            }
          }}
