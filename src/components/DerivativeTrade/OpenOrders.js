...
import { CancelTrade, getOpenOrders } from "../../actions/DerivativeAction";
...
const OpenOrders = () => {
  ...
  useEffect(() => {
    socketContext.socket.on("DerivativeopenOrder", (result) => {
  ...
  return (
    <div className="">
                  ...
                  <tr>
                    ...
                    <td className="text-center">
                      {isNaN(val?.orderValue)
                        ? 0
                        : val?.orderValue?.toFixed(derivativePair.secondDecimal)}{" "}
                      {val?.secondCurrency}
                    </td>
                    <td className="text-center">
                      {isNaN(val?.takeProfitPrice) || val?.takeProfitPrice == 0
                        ? "-"
                        : `${val?.takeProfitPrice?.toFixed(derivativePair.secondDecimal)} ${val?.secondCurrency}`}{" "}

                    </td>
                    <td className="text-center">
                      {isNaN(val?.stopLossPrice) || val?.stopLossPrice == 0
                        ? "-"
                        : `${val?.stopLossPrice?.toFixed(derivativePair.secondDecimal)}  ${val?.secondCurrency}`}{" "}
      ...
      <div className="pt-4 pb-3 d-flex align-items-center justify-content-center">
        {nextPage && openOrder?.length > 0 && count != openOrder?.length ? (
          <button
