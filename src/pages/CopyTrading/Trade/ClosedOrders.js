...
import { getCopyTradeHistory } from "../../../actions/copyTradeAction";
...
const CloseOrders = () => {
    ...
    useEffect(() => {
        socketContext.socket.on("copytradeHistory", (result) => {
    ...
    const fetchTradeHistory = async (CloseOrder, reqData) => {
        try {
            ...
            let { record, status, count, currentPage, nextPage } = await getCopyTradeHistory(data);
