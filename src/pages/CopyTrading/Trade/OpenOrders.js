...
import { getCopyOpenOrders, CancelCopyTrade } from "../../../actions/copyTradeAction";
...
const OpenOrders = () => {
    ...
    useEffect(() => {
        socketContext.socket.on("copyopenOrder", (result) => {
    ...
    const fetchOpenOrder = async (openOrder, reqData) => {
        try {
            ...
            let { record, status, count, currentPage, nextPage } = await getCopyOpenOrders(data);
            console.log(record, "getOpenOrders");
            if (status) {
                console.log(record, "getOpenOrders");
                let isAction = record.find((val) => (val?.userId.toString() == getUser?.userId?.toString()))
                if (!isEmpty(isAction)) {
                    setisAction(true)
                }
    ...
    const CancelOrder = async (orderId) => {
        try {
            ...
            let { status, message } = await CancelCopyTrade(orderId, data, getUser.secretKey);
            if (status) {
                ...
                <table className="table">
                    <thead className="">
                        <tr>
                            ...
                            {isAction ? <th className="text-center">Action</th> : ""}
                        </tr>
                    </thead>
                    <tbody>
                        {openOrder.length > 0 ? (
                            openOrder.map((val, index) => {
                                ...
                                return (
                                    <tr>
                                        ...
                                        {isAction ? <td className="text-center">
                                            <button
